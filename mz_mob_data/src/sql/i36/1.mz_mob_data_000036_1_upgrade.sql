...
--  Updated: 7th August 2018, MZMOB-430/461 [BK]
-- 
--  -------------------------------------------------- 
...
-- DROP TABLE GSMA_NETWORK_EXCEPTIONS CASCADE CONSTRAINTS;
CREATE TABLE GSMA_NETWORK_EXCEPTIONS
(
	ALIAS_TADIG             VARCHAR2(5)   NOT NULL, -- The assigned alias Tadig code issue by a particular roaming hub provider. Note that a different roaming hub provider could issue a different alias Tadig code which points to a common primary Tadig code. 
	TADIG                   VARCHAR2(5)   NOT NULL, -- The Primary TADIG Code of the partner Mobile Network. 
	DummyCountryCode        VARCHAR2(8)   NOT NULL,
	DummyCountry            VARCHAR2(80)  NOT NULL,
	CC                      VARCHAR2(10)  NOT NULL, -- Country code for E.164 formatted destination.
	CountryFromOCS          VARCHAR2(80)  NOT NULL,
	NetworkCountry          VARCHAR2(80)  NOT NULL,
	NetworkName             VARCHAR2(80)  NOT NULL,
	MCC                     VARCHAR2(3)   NOT NULL,
	MNC                     VARCHAR2(3)   NOT NULL,
	VLR_Address_Range_From  VARCHAR2(15)  NOT NULL,
	VLR_Address_Range_To    VARCHAR2(15)  NOT NULL,
	EFF_DATE                DATE          NOT NULL, -- Date the ALIAS translation becomes effective. 
	PROVIDER                VARCHAR2(80),           -- The name of the organization which supplies the Roaming Hub solution. 
	INPUT_FILENAME          VARCHAR2(50)            -- The name of the data file containing the original reference from which the current entry was extracted. 
);

COMMENT ON TABLE GSMA_NETWORK_EXCEPTIONS IS 'Used to translate Roaming Hub assigned ALIAS TADIG codes to the PRIMARY TADIG codes.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.ALIAS_TADIG    IS 'The assigned alias Tadig code issue by a particular roaming hub provider. Note that a different roaming hub provider could issue a different alias Tadig code which points to a common primary Tadig code.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.TADIG          IS 'The Primary TADIG Code of the partner Mobile Network.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.CC             IS 'Country code for E.164 formatted destination.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.MCC            IS 'Mobile Country Code.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.MNC            IS 'Mobile Network Code.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.EFF_DATE       IS 'Date the ALIAS translation becomes effective.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.PROVIDER       IS 'The name of the organization which supplies the Roaming Hub solution.';
COMMENT ON COLUMN GSMA_NETWORK_EXCEPTIONS.INPUT_FILENAME IS 'The name of the data file containing the original reference from which the current entry was extracted.';

CREATE INDEX GMSA_NETWORK_EXCEPTIONS_IDX0 ON GSMA_NETWORK_EXCEPTIONS (ALIAS_TADIG ASC);
CREATE INDEX GMSA_NETWORK_EXCEPTIONS_IDX1 ON GSMA_NETWORK_EXCEPTIONS (TADIG ASC);
CREATE INDEX GMSA_NETWORK_EXCEPTIONS_IDX2 ON GSMA_NETWORK_EXCEPTIONS (EFF_DATE DESC);
CREATE INDEX GSMA_NETWORK_EXCEPTIONS_IDX3 ON GSMA_NETWORK_EXCEPTIONS (DummyCountryCode ASC);
CREATE INDEX GSMA_NETWORK_EXCEPTIONS_IDX4 ON GSMA_NETWORK_EXCEPTIONS (VLR_Address_Range_From DESC, VLR_Address_Range_To ASC);
CREATE INDEX GSMA_NETWORK_EXCEPTIONS_IDX5 ON GSMA_NETWORK_EXCEPTIONS (CountryFromOCS ASC);
...
(
	...
	COUNTRY        VARCHAR2(80)         NOT NULL, -- Country of Network Operator with the assigned TADIG code 
	RELATION_TYPE  NUMBER(3) DEFAULT 1  NOT NULL, -- Roaming Partner Relation Type: Valid values: 1 = Sponsored Roaming (TELE2), 2 = Home Network (TUK), 3 = Roam Hubbing (Comfone), 4 = Unilateral
	GT_RANGE_START VARCHAR2(15),                  -- This column identifies the start of a Global Title Range of Network Nodes owned by a network. (No Padding)
	GT_RANGE_STOP  VARCHAR2(15),                  -- This column identifies the end of a Global Title Range of Network Nodes owned by a network. (No Padding)
	INPUT_FILENAME VARCHAR2(50)         NOT NULL  -- This column is intended to reflect the source data set (file name) from which a particular row is created/updated. 
);
