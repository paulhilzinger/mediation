-- MED-515 - Fix the DB Incrementor for a clean build
--
-- Create the required table
--
CREATE TABLE MZ_CDR_OWNER.DN_ENHANCED_DELETE_EXPIRED 
(
...
-- MED-263 - Update BU Lookup Procedure and BUSINESS_UNIT table in PRODMZN for Sky Business
--
-- INSERT the new BUSINESS_UNIT
--
INSERT INTO MZ_CDR_OWNER.business_units (unit_id, unit_name) VALUES (16, 'SKYBIZ');
...
--BEGIN_PLSQL
create or replace PROCEDURE              DN_ENHANCED_TWIG_UPDATE
AS
...
BEGIN
  ...
  USING (select twig.directory_number, decode(twig.unit_id, 6, twig.join_unit_id, twig.unit_id) unit_id
         ...
         and   decode(twig.unit_id, 6, decode(twig.join_unit_id, 9, 9, 13, 13,16,16, 2), 9, 9, 13, 13,16,16, 2) = can_cease.unit_id) dn -- twig unit_id default to 2 unless 9 for join only
  ...
  and   decode(twig.unit_id, 6, decode(twig.join_unit_id, 9, 9, 13, 13,16,16, 2), 9, 9, 13, 13,16,16, 2) = service.unit_id (+)
  ...
  INSERT INTO MZ_CDR_OWNER.DN_ENHANCED_LOAD
  select service.directory_number,
         service.unit_id,
         service.start_date,
         cast(1 as number),
         service.cease_date,
  ...
  select twig.directory_number, decode(twig.unit_id, 6, twig.join_unit_id, twig.unit_id), kci2.kci2_date, cast(0 as number), null, null
  from nstream_mzn_user.vw_mzn_inflight_prov@SNS_PENFOLD_DB_LINK kci2,
       nstream_mzn_user.vw_mzn_bu_lookup@SNS_PENFOLD_DB_LINK twig
  where twig.directory_number = kci2.directory_number
  and   decode(twig.unit_id, 6, decode(twig.join_unit_id, 9, 9, 13, 13,16,16, 2), 9, 9, 13, 13, 16,16,2) = kci2.unit_id;
  ...
  select twig.directory_number, decode(twig.unit_id, 6, twig.join_unit_id, twig.unit_id), can_prov.kci2_date, cast(0 as number), twig.cancelled_date, cast(1 as number)
  ...
  and   decode(twig.unit_id, 6, decode(twig.join_unit_id, 9, 9, 13, 13,16,16, 2), 9, 9, 13, 13,16,16, 2) = can_prov.unit_id
  ...
  INSERT INTO MZ_CDR_OWNER.DN_ENHANCED_LOAD_CEASE
  select twig.directory_number, decode(twig.unit_id, 6, twig.join_unit_id, twig.unit_id), null, null, cease.cease_request_date, cast(0 as number)
  from nstream_mzn_user.vw_mzn_inflight_cease@SNS_PENFOLD_DB_LINK cease,
       nstream_mzn_user.vw_mzn_bu_lookup@SNS_PENFOLD_DB_LINK twig
  where twig.directory_number = cease.directory_number
  and   decode(twig.unit_id, 6, decode(twig.join_unit_id, 9, 9, 13, 13, 16,16,2), 9, 9, 13, 13,16,16, 2) = cease.unit_id
  and   twig.unit_id != 6;
  ...
  MERGE INTO MZ_CDR_OWNER.DN_BU_LOOKUP_FIXED bul
  USING (SELECT DIRECTORY_NUMBER, UNIT_ID, START_DATE, SKCI_FLAG, CEASE_DATE, CKCI_FLAG FROM MZ_CDR_OWNER.DN_ENHANCED_LOAD) dn
  ON (dn.directory_number = bul.directory_number and dn.unit_id = bul.unit_id)
  WHEN NOT MATCHED THEN
    INSERT (DIRECTORY_NUMBER, UNIT_ID, START_DATE, SKCI_FLAG, CEASE_DATE, CKCI_FLAG)
    VALUES (dn.DIRECTORY_NUMBER, dn.UNIT_ID, dn.START_DATE, dn.SKCI_FLAG, dn.CEASE_DATE, dn.CKCI_FLAG);
