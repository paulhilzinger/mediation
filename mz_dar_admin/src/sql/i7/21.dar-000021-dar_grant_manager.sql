...
AS
  ...
  *
  ...
  !
     ...
     1.6        06/02/2017  ARG              - Added missing exception handlers.
                                             - Removed BIN% restriction from cursors.  Not required
                                             - Now processes PROCEDUREs as well.
  ...
  */
  ...
  IS
    ...
    BEGIN
      ...
      IF USER NOT IN ('SYS', 'SYSTEM', 'DAR_ADMIN', 'BSBDEPLOY', p_schema_name)
      THEN
      ...
      THEN
        ...
        LOOP
          ...
          IF r_gen_grants_tabview.object_type IN ('TABLE') THEN
      ...
      IF ( p_execute_grantee IS NOT NULL )
      THEN
        FOR r_gen_grants_job IN c_gen_grants_job( p_schema_name )
        LOOP

          l_statement := l_action||' ALTER ON '||p_schema_name||'.'||r_gen_grants_job.job_name||' '||l_action_prep||' '||p_execute_grantee;
...
GRANT execute ON dar_owner.dar_grant_manager TO dar_execute;
